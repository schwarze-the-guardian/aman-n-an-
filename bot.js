...
client.on("message", message => {
    ...
    return message.channel.send(a.Diğer.tag)
});
...
if (!cezalımı) return;
if (cezalımı == "cezali") {
member.roles.add(a.Cezalar.Jailed)
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove(a.Cezalar.Jailed);
  }, ms(sürejail));
}
})
...
client.on('guildMemberAdd', async(member) => {
let mute = member.guild.roles.cache.find(r => r.name === "⛔️ Susturulmuş");
let mutelimi = db.fetch(`muteli_${member.guild.id + member.id}`)
let süre = db.fetch(`süre_${member.id + member.guild.id}`)
if (!mutelimi) return;
if (mutelimi == "muteli") {
member.roles.add(a.Cezalar.Muted)
...
db.delete(`muteli_${member.guild.id + member.id}`)
    ...
    member.roles.remove(a.Cezalar.Muted);
  }, ms(süre));
}
})
